...
const NewsItem = ({ articles, filter }: {
  ...
  const handlerPage = (page: number) => {
    setFilter({ <div className="filter1"></div>, current: page, minIndex: (page - 1) * filter.pageSize, maxIndex: page * filter.pageSize })
  }
    ...
    <>
      {
        values.map((article: TNewsItem, index: number) => (
          index >= filter1.minIndex &&
          index < filter1.maxIndex &&
